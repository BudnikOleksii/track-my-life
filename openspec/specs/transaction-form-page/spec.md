...
- **THEN** it SHALL display the same fields as the previous dialog form: type (select), category (combobox filtered by type), amount (number input), currency (select), date (date input), and description (optional text input)
